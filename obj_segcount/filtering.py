...
def convolucion2d(imagen, kernel):
    # Convolución 2D implementada con loops y zero-padding
...
def kernel_gaussiano(tamano, sigma):
    # Genera un kernel gaussiano 2D normalizado dado el tamaño y sigma
    mitad = tamano // 2
...
def filtro_mediana(imagen, tamano=3):
    # El filtro de mediana es bueno para eliminar ruido sal y pimienta
    img = imagen.astype(np.float64)
    pad = tamano // 2
