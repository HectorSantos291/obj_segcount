...
def transformacion_log(imagen, c=1.0):
    # s = c * log(1 + r), normalizado a [0, 255]
...
def estiramiento_contraste(imagen, m=128, e=4):
    # s = 1 / (1 + (m/r)^e), normalizado a [0, 255]
    img = imagen.astype(np.float64)
    epsilon = 1e-6
...
def ajuste_lineal(imagen, c=0, d=255):
    # Mapea el rango [min, max] de la imagen al rango [c, d]
    img = imagen.astype(np.float64)
    a = img.min()
    b = img.max()
...
def ecualizacion_histograma(imagen):
    # Calcula histograma -> CDF -> LUT para ecualizar la imagen
